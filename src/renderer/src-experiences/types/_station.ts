...
class Station {
    ...
    doesLabLocationMatch(): QaCheck {
        ...
        if (this.details.labLocation !== useExperienceStore().reportTracker.labLocation) {
            qaCheck.message = `Station lab location did not match expected lab location. Station lab location: ${this.details.labLocation}. Expected lab location: ${useExperienceStore().reportTracker.labLocation}`
            qaCheck.passedStatus = "failed"
            return qaCheck
        }
        if (this.details.labLocation === useExperienceStore().reportTracker.labLocation) {
