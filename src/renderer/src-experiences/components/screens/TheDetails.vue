...
const labLocation = ref(experienceStore.reportTracker['labLocation']);
const technicianName = ref(experienceStore.reportTracker['technicianName']);
const labType = ref(experienceStore.reportTracker['labType'] ?? "Online");
const headsetType = ref(experienceStore.reportTracker['headsetType'] ?? "Vive Pro 2");
...
const status = computed(() => {
  if (experienceStore.reportTracker['labLocation'] === undefined || experienceStore.reportTracker['technicianName'] === undefined) {
    return 'gray';
  }
  else if (experienceStore.reportTracker['labLocation'].length === 0 || experienceStore.reportTracker['technicianName'].length === 0) {
    return 'gray';
  } else {
    return 'success';
  }
...
const saveDetails = async () => {
  ...
  experienceStore.reportTracker['labLocation'] = labLocation.value;
  experienceStore.reportTracker['technicianName'] = technicianName.value;
  experienceStore.reportTracker['labType'] = labType.value;
  experienceStore.reportTracker['headsetType'] = headsetType.value;
}
</script>
