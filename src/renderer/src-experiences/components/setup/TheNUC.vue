...
const { connected } = storeToRefs(experienceStore);
...
async function connectToNuc() {
  ...
  experienceStore.connected = false
...
async function retryStationConnection() {
  experienceStore.stations.forEach(station => {
    experienceStore.sendStationMessage(station.id, {
...
<template>
  <div class="w-full border-2 border-gray-200 rounded-xl p-4 flex flex-col mb-4">
    <div class="flex flex-row justify-between items-center h-full">
      ...
      <span v-if="connectionState === 'success'">{{experienceStore.nucAddress}}</span>
    </div>
    ...
    <div class="w-full border-2 border-gray-200 rounded-xl p-4 flex flex-col mb-4">
      <div class="flex flex-row items-center h-full justify-between">
        <div class="flex flex-row items-center h-full">
          <StatusIcon :status="experienceStore.stations.length === experienceStore.stations.filter(station => station.details).length ? 'green' : 'red'" class="w-16 h-16 mr-8">
            <template v-slot:icon="{ fill }">
              <ComputerSvg :fill="fill" />
            </template>
          </StatusIcon>
          <span class="text-lg font-semibold col-span-4">Connected to {{ experienceStore.stations.filter(station => station.details).length }}/{{ experienceStore.stations.length }} stations</span>
        </div>
        <div v-if="experienceStore.stations.length !== experienceStore.stations.filter(station => station.details).length"
