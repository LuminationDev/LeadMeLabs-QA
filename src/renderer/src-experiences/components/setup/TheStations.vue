...
watch(() => experienceStore.stations, (newStations) => {
    newStations.forEach(station => {
      station.getComputedChecks().forEach((check) => {
        if (!checks.value[check.id]) {
          //Add the check to the report
          experienceStore.addCheckToReportTracker("connection", "station_details",
        ...
        //Update the report
        experienceStore.updateReport("connection", "station_details",
  ...
  <div class="flex flex-col">
    ...
    <table class="w-full border-collapse mt-4">
      <tr class="text-left text-xs bg-gray-100 border border-gray-200">
        ...
        <th class="w-16 text-center p-3" v-for="station in experienceStore.stations">
          S{{station.id}}
        </th>
      </tr>
