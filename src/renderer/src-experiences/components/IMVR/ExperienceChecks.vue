...
const { experienceChecksCompleted, allowRunningExperienceChecks } = storeToRefs(experienceStore);
...
const startTesting = () => {
  experienceStore.startExperienceChecks()
  inProgress.value = true
}

const retryExperience = (experienceIndex) => {
  experienceStore.launchExperienceOnAll(experienceIndex)
}
...
const hasStartedExperienceChecks = computed(() => {
  return experienceStore.experienceChecks.filter(element => {
...
const allHeadsetsConnected = computed(() => {
  ...
  experienceStore.stations.forEach(station => {
...
const getTotalInstalled = (deviceId: string) => {
  const count = experienceStore.experienceChecks.reduce((count, entry) => {
  ...
  const section = experienceStore.reportTracker["imvr"] ||= {};
...
<template>
  ...
  <div v-if="experienceStore.experienceChecks && experienceStore.experienceChecks.length" class="flex flex-col">
    <table class="w-full border-collapse mt-4">
      ...
      <tr v-for="(device, _index) in experienceStore.deviceMap" :key="_index" class="text-sm border border-gray-200">
        <template v-if="device.type === 'station' && experienceStore.isStationVrCompatible(device.id)">
    ...
    <table class="w-full border-collapse mt-4">
      <tr class="text-left text-xs bg-gray-100 border border-gray-200">
        ...
        <th class="w-16 text-center p-3" v-for="device in experienceStore.experienceChecks[0].stations">
          S{{device.id}}
        </th>
        <th/>
      </tr>
      ...
      <tr v-for="(check, index) in experienceStore.experienceChecks" :key="index" class="text-sm border border-gray-200">
