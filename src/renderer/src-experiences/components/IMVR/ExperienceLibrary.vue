...
const uncollectedExperiences = computed(() => {
  return experienceStore.stations.some(entry => {
...
const getCountByKeyword = (deviceId, keyword) => {
  return experienceStore.experienceErrors.reduce((count, entry) => {
...
<template>
  ...
  <table class="w-full border-collapse mt-4">
    <tr class="text-left text-xs bg-gray-100 border border-gray-200">
      ...
      <template v-for="device in experienceStore.deviceMap">
        <th class="w-16 text-center p-3" v-if="device.type === 'station' && experienceStore.isStationVrCompatible(device.id)">
          S{{device.id}}
        </th>
      </template>
    ...
    <tr v-for="(experience, index) in experienceStore.experienceErrors" :key="index" class="text-sm border border-gray-200">
      ...
      <template v-for="(device, _index) in experienceStore.deviceMap" :key="_index">
        <template v-if="experienceStore.isStationVrCompatible(device.id)">
