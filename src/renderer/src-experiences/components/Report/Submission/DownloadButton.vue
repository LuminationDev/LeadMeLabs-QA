...
import PDFStructure from "./PDFStructure.vue";
...
const fileName = `QAReport_ExperienceLaunches_${experienceStore.reportTracker['labLocation']}_${stateStore.getCurrentDate()}`;
...
<template>
  ...
  <div class="flex flex-col hidden" id="report">
    ...
    <p class="text-black mb-1"><span class="font-semibold">Lab Location: </span>{{experienceStore.reportTracker['labLocation']}}</p>
    <p class="text-black mb-1"><span class="font-semibold">Lab Type: </span>{{experienceStore.reportTracker['labType'] ?? "Online"}}</p>
    <p class="text-black mb-2"><span class="font-semibold">Prepared by: </span>{{experienceStore.reportTracker['technicianName']}}</p>
    <p class="text-black mb-2"><span class="font-semibold">Headset Type: </span>{{experienceStore.reportTracker['headsetType']}}</p>

    <p class="text-black mb-2"><span class="font-semibold">Comments:</span></p>
    <div class="bg-gray-50 text-xs" v-for="comment in experienceStore.reportTracker['comments']">
      <div class="py-3">
        <div>
          {{comment.date}}
        </div>
    ...
    <div v-for="device in experienceStore.deviceMap">
    ...
    <PDFStructure v-for="title in experienceStore.getReportTitles" class="mt-5" :parent="<string>title" :key="title"/>
  </div>
</template>
