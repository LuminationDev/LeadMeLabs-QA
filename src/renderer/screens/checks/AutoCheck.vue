...
const updateDeviceMap = (idKey, item, check) => {
  ...
  tempStore.updateReport(<string>route.meta['parent'], <string>checkType.value, info, check.id, idKey);
};
...
const updateDeviceMaps = (check: QaCheckResult) => {
  ...
  check.tablets.forEach(tablet => {
    const foundTablet = tempStore.deviceMap.find(device => device.ipAddress == tablet.ipAddress);
...
watch(() => tempStore.qaGroups, transformData, { deep: true });

/**
 * Consolidate the information to the single page with no memory at te moment/
 */
const filteredChecks = computed(() => {
  const filteredGroups = tempStore.qaGroups.filter(group => group.id === checkType.value);
...
const retryAutoCheck = () => {
  if(tempStore.mostRecentAutoCheck.length === 0) return;

  tempStore.startQa(tempStore.mostRecentAutoCheck);
}
 ...
 */
const autoRun = () => {
  tempStore.readReportData(<string>route.meta['parent'], <string>checkType.value);
  if (typeof checkType.value === "string") {
    tempStore.mostRecentAutoCheck = checkType.value;
  }
  if (tempStore.groupsThatHaveRun.includes(checkType.value)) {
    checking.value = "done";
    return
  }
  ...
  checking.value = "testing";
  tempStore.startQa(checkType.value);
}
      ...
      <div class="flex flex-col">
        ...
        <table class="w-full border-collapse mt-4">
          <tr class="text-left text-xs bg-gray-100 border border-gray-200">
            ...
            <th class="w-16 text-center p-3" v-for="device in tempStore.deviceMap">
          ...
          <!--Table will not be built if NUC connection has not been made, tempStore.buildQA is triggered on response-->
